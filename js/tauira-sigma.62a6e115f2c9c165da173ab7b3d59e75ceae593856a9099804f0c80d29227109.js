...
`
  ...
  // <stdin>
  var DEFAULT_NODE_COLOR = "#666666";
  var DEFAULT_EDGE_COLOR = "#cccccc";
  var FILTER_IDS = {
  ...
  function enableFullscreen(container, renderer) {
    ...
    btn.addEventListener("click", () => {
      console.log("\u{1F525} Fullscreen clicked");
  ...
  function updateDetails(panel, kind, id, attrs) {
    ...
    if (kind === "node") {
      ...
      ${attrs.contentURL ? `<p><a href="${escapeHtml(attrs.contentURL)}">Open related content</a></p>` : ""}
    `;
      return;
    }
    if (kind === "edge") {
  ...
  async function initTauiraSigma(container, dataPath, detailsSelector = "#tauira-graph-details") {
    ...
    try {
      let normalizeEdgeWeights = function(graph2) {
        graph2.forEachEdge((edge, attrs) => {
          const w = attrs.weight || 0.5;
          const normalized = 0.3 + w * 0.5;
          graph2.setEdgeAttribute(edge, "weight", normalized);
          graph2.setEdgeAttribute(edge, "size", normalized * 2);
        });
      }, addCrossEncounterEdges = function(graph2) {
        const nodes = graph2.nodes();
        for (let i = 0; i < nodes.length; i++) {
          for (let j = i + 1; j < nodes.length; j++) {
            const a = nodes[i];
            const b = nodes[j];
            const aAttrs = graph2.getNodeAttributes(a);
            const bAttrs = graph2.getNodeAttributes(b);
            const aEnc = new Set(aAttrs.encounterRefs || []);
            const bEnc = new Set(bAttrs.encounterRefs || []);
            const shared = [...aEnc].some((e) => bEnc.has(e));
            if (!shared && !graph2.hasEdge(a, b) && Math.random() < 0.15) {
              const edgeId = `bridge-${a}-${b}`;
              if (graph2.hasEdge(edgeId)) continue;
              graph2.addEdgeWithKey(edgeId, a, b, {
                size: 0.5,
                weight: 0.15,
                color: "#eeeeee",
